// Package fs implements extensions to go's fs.FS to work around its limitations
package fs
...
// RootFS defines an interface that extends go's fs.FS with a mechanism for working with absolute paths
...
func NewFromDir(root string) (RootFS, error) {
...
func NewFromFS(fs fs.FS) RootFS {
	root := "/"
